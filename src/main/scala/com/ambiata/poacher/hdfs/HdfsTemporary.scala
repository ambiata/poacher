...
case class HdfsTemporary(seed: String) {
  private val step: AtomicInteger = new AtomicInteger(0)

  def path: Hdfs[Path] = {
    val base = uniqueDirPath
    val incr = step.incrementAndGet.toString
    val path = new Path(new Path(base.path, incr), seed)
    val msg = s"Hdfs($path)"
    addCleanupFinalizer(new Path(base.path), msg) >>
...
object HdfsTemporary {
  ...
  def random: HdfsTemporary =
    HdfsTemporary(java.util.UUID.randomUUID().toString)
}
